...
function candidateRows(inbox) {
  ...
  return inbox.candidates.map((candidate) => {
    const mapping = candidate.dateMapping === 'exact' ? '明确映射（仍待用户确认）' : candidate.dateMapping === 'range' ? '日期范围，不进入日级统计' : '日期未知，不进入日级统计';
    const status = candidate.status === 'confirmed_date' ? `已确认活动日 ${candidate.confirmedActivityDate} · 仅为时间事实候选` : candidate.status === 'rejected' ? '已驳回' : '待核候选';
...
export function renderTimeAttentionPage(app) {
  ...
  <section class="time-block time-readonly"><header><div><span>V2 CANDIDATE INBOX</span><h2>时间候选收件箱</h2></div><p>${candidateInbox.pendingCount} 待核 · ${candidateInbox.confirmedDateCount} 已确认日期 · validated_fact 仍为 0</p></header><div class="time-import-actions"><label class="time-import-button">导入 V2 JSON / JSONL<input id="timeCandidateFile" type="file" accept=".json,.jsonl,application/json"></label><small>unknown / range 只展示；用户确认明确活动日后也只升级为时间事实候选。</small></div><div class="time-task-list">${candidateRows(candidateInbox)}</div></section>
