...
function isCandidateBatch(intake) {
  return isObject(intake?.data)
    && Array.isArray(intake.data.candidates)
    && Object.keys(intake.data).every((key) => key === 'candidates');
}
...
export function buildFeedbackDailyIntakeReceipt(intake = {}, {
  ...
  if (isCandidateBatch(intake)) {
    ...
    const receipt = Object.freeze({
      status: 'processed',
      idempotencyKey: `${receiptBaseKey}:feedback-receipt:processed:v1`,
      projection: Object.freeze({
        payloadKind: 'candidate_batch',
        candidateCount: intake.data.candidates.length,
        candidateOutcome: 'candidate_read_only',
        inputGaps: Object.freeze([]),
      }),
    });
    return Object.freeze({
      receipt,
