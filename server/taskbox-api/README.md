...
The five-system candidate API was merged in PR #2 on 2026-08-12. Its static consumers are live, but the API release is not production-verified until `verify-system-candidates-production.sh` reports `200 / 401 / 204 / 200` and the daily-review outbox has been replayed. Server-side deployment uses `deploy-system-candidates-release.sh`; it stops the service before copying SQLite/WAL/SHM, preserves `/etc/taskbox-api.env` and `data/`, and prints the exact rollback snapshot. Use `rollback-system-candidates-release.sh` only with that printed snapshot; database restoration additionally requires `RESTORE_TASKBOX_DATABASE=1`.
