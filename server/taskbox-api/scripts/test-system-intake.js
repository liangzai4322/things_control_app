...
(async () => {
  try {
    ...
    const healthSnapshot = {
      snapshotId: 'health-scope-snapshot', schemaVersion: 1, date: '2026-09-03', publishedAt: '2026-09-03T12:00:00.000Z',
      healthState: { state: 'green', availableCapacity: 1, confidence: 0.8, evidenceRefs: [] },
      timeSystem: { availableCapacity: 1, constraints: [], evidenceRefs: [], privacy: 'capacity_and_constraints_only' },
      dailyReview: { state: 'green', availableCapacity: 1, confidence: 0.8, missing: [], conflicts: [], sourceSummary: {}, activeInterventions: [], privacy: 'minimum_health_snapshot' },
      boundaries: { createsTasks: false, writesCalendar: false, medicalDiagnosis: false, requiresUserApprovalForDownstreamAction: true },
    };
    const genericSnapshotWrite = await request('/v1/health/snapshots', 'POST', healthSnapshot);
    if (genericSnapshotWrite.status !== 401) throw new Error('generic TaskBox token must not write health snapshots');
    const crossSystemSnapshotWrite = await request('/v1/health/snapshots', 'POST', healthSnapshot, { authToken: consumerTokens.execution });
    if (crossSystemSnapshotWrite.status !== 403) throw new Error('non-health daily identity must not write health snapshots');
    const healthSnapshotWrite = await request('/v1/health/snapshots', 'POST', healthSnapshot, { authToken: consumerTokens.health });
    if (healthSnapshotWrite.status !== 200 || healthSnapshotWrite.data.created !== true) throw new Error('health identity must write health snapshots');
    const unknownSnapshotField = await request('/v1/health/snapshots', 'POST', { ...healthSnapshot, privateNote: 'must-not-leak' }, { authToken: consumerTokens.health });
    if (unknownSnapshotField.status !== 400) throw new Error('health snapshot unknown fields must be rejected');
