...
function importTaskbox() {
  ...
  if (cleanSettings.dailyQuote) {
    const updatedAt = cleanSettings.dailyQuoteUpdatedAt || now();
    upsertMeta.run({
      key: 'daily_quote',
      value_json: json({
        current: cleanSettings.dailyQuote,
        updatedAt,
        history: cleanSettings.dailyQuoteHistory || [{ text: cleanSettings.dailyQuote, updatedAt }],
      }),
      updated_at: now(),
    });
  }
