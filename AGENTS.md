...
- `docs/hq-primary-action-system-loop-v2.md` is the single source of truth for the primary-action seat, ROI candidate engine, and subsystem fact-to-action loop. P0-P3 are in production; P3 Build ID `dca5c12098ba` was published by Pages workflow `31302177865` attempt 2, and P4 remains proposed. `js/hq-systems.js` is the lightweight registry: it distinguishes L0 entry, L1 read-only and L2 controlled-write contracts, never reports an unread source as healthy, and promotes only `blocked / needs_action` mainline facts to the existing ROI engine. TaskBox and daily review remain the existing L2 controlled paths; no API runtime or database schema change is part of P3. P3 full tests, build, 390px/1440px, contract-sheet, navigation, console and production-asset checks passed. The P1 server rollback point remains `/opt/taskbox-api/backups/p1-action-seat-20260809T022214Z`.
