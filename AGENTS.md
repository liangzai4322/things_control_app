...
- `docs/hq-primary-action-system-loop-v2.md` is the single source of truth for the primary-action seat, ROI candidate engine, and subsystem fact-to-action loop. P0 was completed end to end on 2026-08-07: preserve mutation waiting, persistent mutation outbox/replay, task-version reconciliation, cloud tombstone authority, stale-render guards, local-only offline records, and explicit `primaryTaskId: null` clearing. Production completion, cancel-completion, cross-source cache, weak-network route switching, offline reload/reconnect, authenticated health, CORS preflight, and null-clear/restore probes passed. P1 was released to production on 2026-08-09: `currentActionTaskId` is the current action seat, `primaryTaskId` remains the original strategic commitment, completed seat tasks move into today outcomes via `completionReceipt`, and daily-brief sync fences reject stale generation/sequence replays. The P1 server rollback point is `/opt/taskbox-api/backups/p1-action-seat-20260809T022214Z`. P2-P4 remain proposed behavior and must not be treated as current fields or production APIs.
- The downstream Task Hub bridge lives in `D:\note_new\06-日常输入_输出\.agents\skills\任务中枢\scripts\task_hub_bridge.py`. It creates one TaskBox task through the record API; HQ reads that shared task and only receives a daily-brief update for an explicit primary or maintenance commitment.
