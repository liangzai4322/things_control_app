...
- `docs/hq-primary-action-system-loop-v2.md` is the single source of truth for the primary-action seat, ROI candidate engine, and subsystem fact-to-action loop. P0-P1 are in production; the P1 server rollback point is `/opt/taskbox-api/backups/p1-action-seat-20260809T022214Z`. P2 release candidate `a485fa88a115` is locally complete: `js/hq-candidates.js` ranks eligible existing tasks and native mainline signals with explainable ROI, threshold, limit and cooldown; confirmation reuses an existing task or creates exactly one task with `hq-candidate:<dedupeKey>` as the server-idempotent sync key. Daily-brief cache patches merge with the existing brief; explicit `primaryTaskId: null` remains the authoritative clear. P2 full tests, API idempotency, 390px/1440px and browser double-confirm checks passed. Until Pages verification finishes, treat P2 as releasing rather than production-complete; P3-P4 remain proposed.
