...
- `docs/hq-primary-action-system-loop-v2.md` is the single source of truth for P0-P6. P0-P4 are in production. P5-P6 add a read-only approved weekly bet, project resource bias, weekly system-efficiency metrics, and a 14-day governance gate; missing values remain unknown and recommendations never mutate systems or TaskBox. The 2026-08-23 release candidate Build ID is `8956dbc8f134`, with full tests and 1440px/390px browser checks passed. P4 API promotion boundaries remain unchanged: only approved daily proposals can promote when both the request and `HQ_PROPOSAL_PROMOTION_ENABLED=1` allow it; weekly experiments and monthly bets remain strategic objects.
