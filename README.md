...
2026-08-23 使命系统Beta最小正确版本开始实施：新增独立`/v1/mission/*`记录级云端账本，草稿、正式版本、候选和事件保持稳定ID、内容哈希、revision冲突保护与不可变审计；浏览器仍本地优先并保留待同步载荷。正式版本可由`explicit_user`或精确`mission-hq-specific-actions-2026-08-23`长期授权发布，AI不能自授权。HQ继续只读正式`activeVersion`，待发布草稿只显示差异数量；使命域不写TaskBox任务、主线或盒子。
