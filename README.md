...
五系统历史初始化使用私有的“首轮30日-V2”包：HQ固定入口可一次原子导入使命、健康、时间、执行、反馈五个本地store，失败全部回滚，重复导入保持幂等并保留用户已有裁决。初始化只写候选层：使命39条准入候选、健康84、时间135、执行375、反馈790条观察/主张、22个语义簇、42个模式候选和5个校准提案；validated fact、TaskBox任务、激活实验/规则均为0。初始化包含历史原文与本机引用，保持私有，不进入Git仓库、Pages构建或服务器。
