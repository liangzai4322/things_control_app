...
- `docs/hq-primary-action-system-loop-v2.md`: 主动作席位、投产比候选、统一子系统接入、复盘校准与资源治理的唯一实施方案；P0–P6 已完成生产发布。
...
2026-08-23 P5–P6 已完成生产发布：HQ 当前行动上方增加“本周唯一赌注”，只读取已批准月度押注；项目中心显示计划投入、实际投入、外部结果和资源判断；“系统白痴指数”读取周省六个可选治理指标，满 14 个观测日且五项指标齐全后才建议保留、简化或停止。缺失数据保持未知，草稿不进入正式赌注，治理建议不自动删功能或派任务。PR #12 合并提交`4237b97`，Pages 工作流`32634346744`发布 Build ID`8956dbc8f134`；全量测试、周期桥接、线上分块和1440px/390×844无溢出、无控制台错误验收通过。

2026-08-23 审批交互增量：日动作同意后直接选盒并连续批准/晋升；拒绝改为一键操作，进入折叠回收池并支持6秒撤销或长期恢复，审计记录保留。

2026-08-23 使命系统Beta最小正确版本已发布：新增独立`/v1/mission/*`记录级云端账本，草稿、正式版本、候选和事件保持稳定ID、内容哈希、revision冲突保护与不可变审计；浏览器仍本地优先并保留待同步载荷。正式版本可由`explicit_user`或精确`mission-hq-specific-actions-2026-08-23`长期授权发布，AI不能自授权。HQ继续只读正式`activeVersion`，待发布草稿只显示差异数量；使命域不写TaskBox任务、主线或盒子。提交`d62129c`由Pages工作流`32632062875`和API工作流`32632070021`完成生产发布，Build ID为`fa9bb4d8a669`，API回滚点为`/opt/taskbox-api/backups/system-candidates-20260823T094954Z`。

2026-08-23 反馈系统总部完成本地/云端基线核对：保留 `deploy/main` 上较新的反馈 schema v3 与四层候选合同，不用旧 P1 文件覆盖；反馈继续独立使用本地 store，并只通过 `js/feedback-hq-port.js` 向 HQ 输出 L1 摘要。反馈总部可独立维护和发布反馈域代码，但导入或 AI 推导的规则、实验及跨系统提案仍不能自授权、直接改写目标系统或创建 TaskBox 任务。本次发布前全量测试、专项测试、Build ID `1bf08b491371`、1440px/390px 页面宽度及控制台检查均通过，未修改 API 运行代码或生产数据库。

2026-08-23 健康页收件箱收敛：日省候选是跨系统传输/审计缓冲，明确用户健康事实由健康同步链路自动入库；健康页只保留“健康事实待确认”作为外部/历史/日期不明材料的人工准入层，避免日省后重复点击。

2026-08-12 五系统V3已正式部署。使命、健康、时间、执行、反馈各自拥有独立页面、状态和HQ端口；人生参谋部只通过`js/five-system-hq-ports.js`消费标准摘要。执行是独立L2系统，TaskBox仅是其唯一任务与完成事实引擎。全量测试和双视口六页面验收通过；Pages工作流`31556529819`发布Build ID`6ee91e341ff7`。2026-09-02 后续受控写入统一使用独立`/v1/execution/*`生产合同；候选仍不能自授权，通用任务写路径不向执行系统开放。

2026-08-13 日省候选链已完成生产闭环：PR #2 的 Pages Build ID`e95fe81c2a02`继续提供五个独立收件箱，生产 API 的`system_candidates`表与批量/读取/决策路由已发布。线上验收为认证健康`200`、未认证`401`、CORS`204`、候选路由`200`；2026-08-11 outbox 首次创建3条，重复投递3条全部 unchanged，使命1条、执行2条，其余系统0条且隔离读取通过。服务器回滚点为`/opt/taskbox-api/backups/system-candidates-20260812T163954Z`。候选仍只允许保留/忽略，不会自动写入五系统事实层。

五系统历史初始化使用私有的“首轮30日-V2”包。2026-08-13起，HQ固定入口会在用户明确选择文件后原子发布“五系统历史基线V1”：使命39条纳入基线、健康12条非提案单日记录生成可追溯历史Observation并保留72条上下文、时间22条非提案单日记录成为历史事实并保留113条上下文、执行375条成为历史执行记录但不创建当前TaskBox任务、反馈42个模式进入observed且5个校准提案继续保持proposed。每次发布形成版本，保留整批发布前快照，可一键回退上一版；失败全部回滚。初始化包含历史原文与本机引用，保持私有，不进入Git仓库、Pages构建或服务器。已上线的服务器接口仍只是“每日新增候选”收件箱，不等于五个内部store已迁移到服务器。

该可回退基线能力已由PR #7于2026-08-13发布：合并提交`47624b3`，Pages工作流`31660897657`成功，线上Build ID为`0edb9e215060`。

跨浏览器初始化不再要求用户手工选择文件：生产API通过认证路由`GET /v1/system-baseline/current`读取服务器私有包，HQ发现当前浏览器尚未发布基线时自动拉取并发布V1。私有包只存在服务器受限目录，不进入Git、Pages或未认证URL；没有配置API Token时仍保留本地文件入口作为兜底。

该自动初始化链已由PR #8于2026-08-13发布：合并提交`7a9c536`，Pages工作流`31712549246`成功，线上Build ID`56ce0c452a92`；生产基线接口认证200、未认证401，API回滚点为`/opt/taskbox-api/backups/system-candidates-20260813T145401Z`。
