...
## HQ 跨系统协作收件箱（2026-09-01）

各子系统继续独立拥有事实、规则和写权限；HQ只读取最小投影与候选计数。日省在 `reviewDate` 生成的停止做/继续保持写入下一日 `briefDate`，HQ分别展示“今日事实”和“本次复盘生成的明日预览”，不得复制改写日期归属。

HQ将子系统状态统一呈现为：无数据、候选待处理、正式状态正常、数据过期、发现异常或仅提供入口。协作收件箱只聚合会影响当前判断且需要用户补充的缺口；普通 unknown、无变化和单纯候选不形成打扰式通知。周/月视图只读取最近五系统投影并展示输入就绪度，不触发子系统重算历史。
