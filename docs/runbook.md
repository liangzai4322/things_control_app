...
20. Assistant Gateway worker 发布前确认 Notification Hub 的 lease-bound reply 路由已上线，运行`npm run test:assistant-gateway`。发布脚本只用 ingress 专用身份对不存在消息执行`404`认证探针，再以独立无登录用户启动`assistant-gateway.service`。Echo阶段只处理“测试”或带时间标识的“测试-...”文本，回声成功后才ack；其他消息延后一小时，不调用HQ或TaskBox。
