...
date: 2026-08-03
status: P0-P1 已完成生产发布与线上验收；P2 于 2026-08-09 完成本地发布候选并进入发布流程；P3-P4 待实施
sources:
...
本轮只定义产品规则、数据流、接口边界、实施顺序与验收标准，不修改前端、服务器、数据库、日省 skill 或生产数据。
...
| 系统健康与接入配置 | `system_registry` | 读取与管理 | 无 |
...
2026-08-09 本地发布候选：已实现现有 TaskBox 任务的资格过滤、九维规则评分、55 分门槛、最多三项稳定排序、四小时跳过冷却、人工选择和确认接棒。主线系统的`blocked / needs_action`事实成为首个原生`ActionCandidate`输入；确认时先按任务 ID、`candidateDedupeKey`和`hq-candidate:<dedupeKey>`查重，不存在才在重要盒创建一条关联原主线的任务。真实浏览器通过“原始承诺完成→战果→候选跳过→新候选排序→确认接棒”和“无下一步主线→原生候选→双击确认→仅一条任务→项目恢复健康”；390px/1440px 无横向溢出或控制台错误。缓存部分更新保留原始承诺，显式`primaryTaskId: null`仍是唯一完整清空语义。专项/全量/API 幂等测试与 Build ID `a485fa88a115` 通过，P2 正在发布。
...
- [ ] 每个候选有来源、完成标准、分数和推荐理由。
- [ ] 已完成、删除、暂停、未释放、硬阻塞任务被过滤。
- [ ] 数据过期或来源未知会降低可信度或取消资格。
- [ ] 同分排序稳定，接近同分时交给用户选择。
- [ ] 系统建设任务必须说明现实收益才获得高分。
...
P1 已于 2026-08-09 完成生产发布“行动席位与今日战果”：

1. 固化今日原始战略承诺与当前行动的分离语义。
2. 增加今日战果与完成回执入口。
3. 设计完成后候选空状态，但暂不实现投产比评分引擎。

P1 发布后仍不实现投产比推荐，也不增加新子系统写回或生产数据库字段；P2 才处理候选评分与自动接棒。
