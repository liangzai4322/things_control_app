...
const legacyCandidateBatch = {
  ...intake,
  id: 'legacy-attention-candidate-batch',
  reviewDate: '2026-09-01',
  observationPeriod: { activity_start: '2026-08-27', activity_end: '2026-09-01' },
  data: { candidates: [{ candidateId: 'attention-candidate-1', systemId: 'time', statement: '候选时间事项' }] },
};
assert.deepEqual(validateAttentionIntake(legacyCandidateBatch), { ok: true, payloadKind: 'candidate_batch' });
const legacyReceipt = buildAttentionReceipt(legacyCandidateBatch);
assert.equal(legacyReceipt.status, 'processed');
assert.equal(legacyReceipt.projection.payloadKind, 'candidate_batch');
assert.equal(legacyReceipt.projection.candidateOutcome, 'candidate_read_only');
assert.equal(legacyReceipt.projection.candidateCount, 1);
