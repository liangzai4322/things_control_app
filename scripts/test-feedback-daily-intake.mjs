...
const legacyCandidateBatch = {
  ...intake,
  id: 'intake-feedback-candidate-batch',
  data: { candidates: [{ candidateId: 'feedback-candidate-1', systemId: 'feedback', statement: '候选反馈事项' }] },
};
const legacyCandidateReceipt = buildFeedbackDailyIntakeReceipt(legacyCandidateBatch);
assert.equal(legacyCandidateReceipt.receipt.status, 'processed');
assert.deepEqual(legacyCandidateReceipt.receipt.projection, {
  payloadKind: 'candidate_batch', candidateCount: 1, candidateOutcome: 'candidate_read_only', inputGaps: [],
});
